...
import { DV_STATUSES, DV_FILTERS, dvStatusLabel } from "@/lib/dv";
          ...
          <tbody className="divide-y divide-gray-100">
            {dvs.map((d) => (
              <tr key={d.id} className={d.status === "Void" ? "opacity-50" : "hover:bg-gray-50"}>
                ...
                <td className={`table-td text-right ${d.status === "Void" ? "text-gray-400" : d.amount - d.paidAmount > 0.005 ? "text-amber-700" : "text-gray-400"}`}>{d.status === "Void" ? "—" : peso(Math.max(0, d.amount - d.paidAmount))}</td>
                <td className="table-td"><StatusBadge status={dvStatusLabel(d.status)} /></td>
              </tr>
            ))}
            {!dvs.length && <tr><td colSpan={10} className="p-8 text-center text-sm text-gray-500">No disbursement vouchers yet.</td></tr>}
          </tbody>
          {dvs.length > 0 && <tfoot className="border-t border-gray-200 bg-gray-50 font-bold"><tr><td className="table-td" colSpan={7}>AUTHORISED, NOT YET PAID</td><td /><td className="table-td text-right text-amber-700">{peso(open)}</td><td /></tr></tfoot>}
        </table>
      </div>
